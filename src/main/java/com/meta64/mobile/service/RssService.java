...
public class RssService {
	...
	// * intermittently. If we didn't do it this way the only other alternative would be to perform
	// a
	...
	// feedsRootNode = JcrUtil.ensureNodeExists(session, "/" + JcrName.RSS + "/", JcrName.RSS_FEEDS,
	// "# RSS Feeds");
	...
	//
	// /*
	// * NOTE: We use a simple recursion algorithm here, but it would be more efficient to actually
	// do
	// * a query for the rssfeed type itself, and just process the results of the query. Leaving
	// that
	...
	// * PlayerInfo paris from the hashmap and persist them permanently into the db so that even
	// after
	// * a server restart the time offsets where each person's podcast was left off will still be
	// * available after reboot. For now a server restart/redeploy wipes out everyone's info
	// regarding
