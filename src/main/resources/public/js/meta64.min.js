...
d!==g.value&&(e.push({name:g.name,value:d}),f++)}else break;c++}0<f?(util.json("saveNode",{nodeId:k.editNode.id,properties:e,sendNotification:a},b),a=!1):alert("You didn't change any information!")},moveNodeUp:function(a){var b=meta64.uidToNodeMap[a];b?(a=meta64.getOrdinalOfNode(b),console.log("ordinal="+a),-1==a&&0>=a||util.json("setNodePosition",{parentNodeId:meta64.currentNodeId,nodeId:b.name,siblingId:meta64.currentNodeData.children[a-1].name},m)):console.log("idToNodeMap does not contain "+a)},
moveNodeDown:function(a){var b=meta64.uidToNodeMap[a];b?(a=meta64.getOrdinalOfNode(b),console.log("ordinal="+a),-1==a&&a>=meta64.currentNodeData.children.length-1||util.json("setNodePosition",{parentNodeId:meta64.currentNodeData.node.id,nodeId:meta64.currentNodeData.children[a+1].name,siblingId:b.name},m)):console.log("idToNodeMap does not contain "+a)},fullRepositoryExport:function(){util.json("exportToXml",{nodeId:"/"},e)},openExportPg:function(){meta64.changePage(exportPg)},openRenameNodePg:function(){meta64.changePage(renameNodePg)},
...
var a="",b=0;meta64.fieldIdToPropMap={};if(k.editNode)$.each(k.editNode.properties,function(e,c){if(render.allowPropertyToDisplay(c.name)){var f="editNodeTextContent"+b;meta64.fieldIdToPropMap[f]=c;var d=c.values&&0<c.values.length,g=render.isReadOnlyProperty(c.name),l=render.isBinaryProperty(c.name);if(g||l)f="RdOnly_"+f;var h="";if(!g&&!l){var h=render.makeTag("a",{onClick:"props.clearProperty('"+f+"');","class":"ui-btn ui-btn-inline ui-icon-back ui-btn-icon-left"},"Clear"),k="",m="";c.name!==jcrCnst.CONTENT&&
(m=render.makeTag("a",{onClick:"props.deleteProperty('"+c.name+"');","class":"ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-left"},"Del"),k=render.makeTag("a",{onClick:"props.addSubProperty('"+f+"');","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Add Multi"));h=k+h+m;h=0<h.length?render.makeHorizontalFieldSet(h,"property-edit-button-bar"):""}d?(console.log("Property multi-type: name="+c.name+" count="+c.values.length),h+=props.makeMultiPropEditor(f,c,g,l)):(console.log("Property single-type: "+
c.name),h+=render.makeTag("label",{"for":f},render.sanitizePropertyName(c.name)),d=l?"[binary]":c.value,h=g||l?h+render.makeTag("textarea",{id:f,readonly:"readonly",disabled:"disabled"},d?d:""):h+render.makeTag("textarea",{id:f},d?d:""));a+=render.makeTag("div",{"class":"ui-field-contain property-edit-field"},h);b++}else console.log("Hiding property: "+c.name)});else var e=render.makeTag("label",{"for":"newNodeNameId"},"New Node Name")+render.makeTag("textarea",{id:"newNodeNameId"},""),a=a+render.makeTag("div",
{"class":"ui-field-contain property-edit-field"},e);util.setHtmlEnhanced($("#propertyEditFieldContainer"),a);e=k.editingUnsavedNode?"You may leave this field blank and a unique ID will be assigned. You only need to provide a name if you want this node to have a more meaningful URL.":"";$("#editNodeInstructions").html(e);util.setVisibility("#addPropertyButton",!k.editingUnsavedNode);e=k.editNode&&k.editNode.id&&!k.editNode.id.startsWith("/");util.setVisibility("#makeNodeReferencableButton",k.editNode&&
!e&&!k.editingUnsavedNode)},insertNode:function(a){if(d=meta64.currentNode){var b=null;if(b=a?meta64.uidToNodeMap[a]:meta64.getHighlightedNode())k.nodeInsertTarget=b,k.startEditingNewNode()}else console.log("Unknown parent")},createSubNodeUnderHighlight:function(){(d=meta64.getHighlightedNode())?(k.nodeInsertTarget=null,k.startEditingNewNode()):alert("Tap a node to insert under.")},replyToComment:function(a){k.createSubNode(a)},createSubNode:function(a){if(a){if(d=meta64.uidToNodeMap[a],!d){console.log("Unknown nodeId in createSubNode: "+
...
a("Find Shared Subnodes","findSharedNodesButton","share.findSharedNodes();"),f=d("Sharing",f),g=a("Toggle Properties","propsToggleButton","props.propsToggle();")+a("Refresh","refreshPageButton","view.refreshPage();")+a("Show URL","showFullNodeUrlButton","render.showNodeUrl();")+a("Server Info","showServerInfoButton","view.showServerInfo();"),g=d("View Options",g),b=render.makeTag("div",{style:"margin: 0; min-width: 300px; position: fixed; top: 2em; left: 2em;","data-role":"collapsible-set"},b+e+f+
g);util.setHtmlEnhanced($("#popupMenuPg"),b)},init:function(){var a=util.getPropertyCount(meta64.selectedNodes),e=meta64.getHighlightedNode(),f=!util.nullOrUndef(edit.nodesToMove)&&!b.isAnonUser;util.setEnablement($("#changePasswordPgButton"),!meta64.isAnonUser);util.setEnablement($("#accountPreferencesPgButton"),!meta64.isAnonUser);util.setEnablement($("#insertBookWarAndPeaceButton"),meta64.isAdminUser&&null!=e,meta64.isAdminUser);util.setEnablement($("#fullRepositoryExport"),meta64.isAdminUser,
...
edit.editNode.properties.length;c++)if(a===edit.editNode.properties[c].name){edit.editNode.properties.splice(c,1);break}},makeMultiPropEditor:function(a,c,e,f){var d="",l=c.values;l&&0!=l.length||(l=[],l.push(""));for(var h=0;h<l.length;h++){console.log("prop multi-val["+h+"]="+l[h]);var n=a+"_subProp"+h,d=d+render.makeTag("label",{"for":n},(0==h?c.name:"*")+"."+h),m=f?"[binary]":l[h],d=f||e?d+render.makeTag("textarea",{id:n,readonly:"readonly",disabled:"disabled"},m?m:""):d+render.makeTag("textarea",
{id:n},m?m:"")}return d},setPreferredPropertyOrder:function(a){if(a){var c=[];$.each(a,function(a,b){b.name===jcrCnst.CONTENT?c.unshift(b):c.push(b)});return c}},renderProperties:function(a){if(a){var c="<table data-role='table' class='prop-table ui-responsive ui-shadow table-stripe property-text'>",e=0,c=c+"<thead><tr><th></th><th></th></tr></thead>",c=c+"<tbody>";$.each(a,function(a,b){if(render.allowPropertyToDisplay(b.name)){var d=render.isBinaryProperty(b.name);e++;c+="<tr class='prop-table-row'>";
c+="<td class='prop-table-name-col'>"+render.sanitizePropertyName(b.name)+"</td>";c=d?c+"<td class='prop-table-val-col'>[binary]</td>":b.values?c+("<td class='prop-table-val-col'>"+props.renderPropertyValues(b.values)+"</td>"):c+("<td class='prop-table-val-col'>"+render.wrapHtml(b.htmlValue?b.htmlValue:b.value)+"</td>");c+="</tr>"}else console.log("Hiding property: "+b.name)});return 0==e?"":c+="</tbody></table>"}},getNodeProperty:function(a,c){if(!c||!c.properties)return null;for(var e=0;e<c.properties.length;e++){var d=
c.properties[e];if(d.name===a)return d}return null},getNodePropertyVal:function(b,c){var e=a.getNodeProperty(b,c);return e?e.value:null},isNonOwnedNode:function(b){b=a.getNodePropertyVal(jcrCnst.CREATED_BY,b);return null!=b&&b!=meta64.userName},isNonOwnedCommentNode:function(b){b=a.getNodePropertyVal(jcrCnst.COMMENT_BY,b);return null!=b&&b!=meta64.userName},renderProperty:function(b){return b.values?a.renderPropertyValues(b.values):b.value&&0!=b.value.length?render.wrapHtml(b.htmlValue):""},renderPropertyValues:function(a){var c=
"<div>";$.each(a,function(a,b){c+=render.wrapHtml(b)+cnst.BR});return c+="</div>"}};console.log("Module ready: props.js");return a}();console.log("running module: render.js");
...
var share=function(){var d=function(a){e.populateSharingPg(a)},a=function(a){srch.searchNodesResponse(a)},b=function(a,b){var e="";$.each(b.privileges,function(b,c){var d=render.makeTag("a",{onClick:"share.removePrivilege('"+a+"', '"+c.privilegeName+"');","class":"ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-left"},"Remove"),d=render.makeHorizontalFieldSet(d),d=d+("<b>"+a+"</b> has privilege <b>"+c.privilegeName+"</b> on this node.");e+=render.makeTag("div",{"class":"privilege-entry"},d)});return e},
c=function(a){util.json("getNodePrivileges",{nodeId:e.sharingNode.path,includeAcl:!0,includeOwners:!0},d)},e={sharingNode:null,populateSharingPg:function(a){var c="<h2>Node Share Settings</h2>";$.each(a.aclEntries,function(a,e){c+="<h4>User: "+e.principalName+"</h4>";c+=render.makeTag("div",{"class":"privilege-list"},b(e.principalName,e))});c+=render.makeTag("input",{type:"checkbox",name:"allowPublicCommenting",id:"allowPublicCommenting"},"",!1);c+=render.makeTag("label",{"for":"allowPublicCommenting"},
"Allow public commenting under this node.",!0);util.setHtmlEnhanced($("#sharingListFieldContainer"),c);util.setCheckboxVal("#allowPublicCommenting",a.publicAppend);$("#allowPublicCommenting").bind("change",e.publicCommentingChanged)},publicCommentingChanged:function(){var a=$("#allowPublicCommenting").is(":checked");util.json("addPrivilege",{nodeId:e.sharingNode.id,publicAppend:a?"true":"false"},null)},removePrivilege:function(a,b){util.json("removePrivilege",{nodeId:e.sharingNode.id,principal:a,
privilege:b},c)},shareNodeToPersonPg:function(){meta64.changePage(shareToPersonPg)},shareNodeToPerson:function(){var a=$("#shareToUserName").val();a?util.json("addPrivilege",{nodeId:e.sharingNode.id,principal:a,privileges:["read","write","addChildren","nodeTypeManagement"]},e.reloadFromShareWithPerson):alert("Please enter a username")},shareNodeToPublic:function(){console.log("Sharing node to public.");util.json("addPrivilege",{nodeId:e.sharingNode.id,principal:"everyone",privileges:["read"]},e.reload)},
editNodeSharing:function(){var a=meta64.getHighlightedNode();a?(e.sharingNode=a,meta64.changePage(sharingPg)):alert("No node is selected.")},reloadFromShareWithPerson:function(a){util.checkSuccess("Share Node with Person",a)&&meta64.changePage(sharingPg)},reload:function(){console.log("Loading node sharing info.");util.json("getNodePrivileges",{nodeId:e.sharingNode.id,includeAcl:!0,includeOwners:!0},d)},findSharedNodes:function(){var b=meta64.getHighlightedNode();null!=b&&(srch.searchPageTitle="Shared Nodes",
util.json("getSharedNodes",{nodeId:b.id},a))}};console.log("Module ready: share.js");return e}();console.log("running module: user.js");
