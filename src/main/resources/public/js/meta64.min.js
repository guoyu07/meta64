...
{};if(k.editNode)$.each(k.editNode.properties,function(d,e){if(render.allowPropertyToDisplay(e.name)){var c="editNodeTextContent"+b;meta64.fieldIdToPropMap[c]=e;var f=e.values&&0<e.values.length,h=render.isReadOnlyProperty(e.name),g=render.isBinaryProperty(e.name);if(h||g)c="RdOnly_"+c;var l="";if(!h&&!g){var l=render.makeTag("a",{onClick:"props.clearProperty('"+c+"');","class":"ui-btn ui-btn-inline ui-icon-back ui-btn-icon-left"},"Clear"),k="",m="";e.name!==jcrCnst.CONTENT&&(m=render.makeTag("a",
{onClick:"props.deleteProperty('"+e.name+"');","class":"ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-left"},"Del"),k=render.makeTag("a",{onClick:"props.addSubProperty('"+c+"');","class":"ui-btn ui-btn-inline ui-icon-star ui-btn-icon-left"},"Add Multi"));l=k+l+m;l=0<l.length?render.makeHorizontalFieldSet(l,"property-edit-button-bar"):""}f?(console.log("Property multi-type: name="+e.name+" count="+e.values.length),l+=props.makeMultiPropEditor(c,e,h,g)):(console.log("Property single-type: "+e.name),
l+=render.makeTag("label",{"for":c},render.sanitizePropertyName(e.name)),f=g?"[binary]":e.value,l=h||g?l+render.makeTag("textarea",{id:c,readonly:"readonly",disabled:"disabled"},f?f:""):l+render.makeTag("textarea",{id:c},f?f:""));a+=render.makeTag("div",{"class":"ui-field-contain property-edit-field"},l);b++}else console.log("Hiding property: "+e.name)});else var d=render.makeTag("label",{"for":"newNodeNameId"},"New Node Name")+render.makeTag("textarea",{id:"newNodeNameId"},""),a=a+render.makeTag("div",
...
meta64.getSelectedNodeIdsArray();a&&0!=a.length?confirmPg.areYouSure("Confirm Move","Move "+a.length+" node(s) to a new location ?","Yes, move.",function(){k.nodesToMove=a;meta64.selectedNodes={};alert("Ok, ready to move nodes. To finish moving, go select the target location, then click 'Finish Moving'");meta64.refreshAllGuiEnablement()}):alert("You have not selected any nodes. Select nodes to delete first.")},finishMovingSelNodes:function(){confirmPg.areYouSure("Confirm Move","Move "+k.nodesToMove.length+
