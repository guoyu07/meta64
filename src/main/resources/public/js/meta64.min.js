...
selectedNodes:{},currentNodeData:null,currentNode:null,currentNodeUid:null,currentNodeId:null,currentNodePath:null,dialogMap:{},dataObjMap:{},dialogStack:[],updateMainMenuPanel:function(){popupMenuPg.build();popupMenuPg.init()},registerDialog:function(b){a.dialogMap[b.domId]=b},registerDataObject:function(b){b.guid=++a.nextGuid;a.dataObjMap[b.guid]=b},getObjectByGuid:function(b){var c=a.dataObjMap[b];c||console.log("data object not found: guid="+b);return c},runCallback:function(a){var c=meta64.getObjectByGuid(a);
c.callback?c.callback():"function"==typeof c?c():alert("unable to find callback on registered guid: "+a)},inSimpleMode:function(){return a.editModeOption===a.MODE_SIMPLE},goToMainPage:function(b,c){a.jqueryChangePage("mainTabName");c&&(a.treeDirty=!0);b||a.treeDirty?(a.treeDirty?view.refreshTree(null,!0):render.renderPageFromData(),a.refreshAllGuiEnablement()):view.scrollToSelectedNode()},selectTab:function(a){document.querySelector("#mainIronPages").select(a)},changePage:function(b,c){if("undefined"===
typeof b.tabId)return console.log("oops, wrong object type passed to changePage function."),null;if("popup"!=b.tabId){var e=document.querySelector("#mainPaperTabs");e.select(b.tabId)}if("popup"==b.tabId){null==c&&console.log("Warning: No data for popup: "+b.tabId);var e=util.polyElm("modalsContainer"),d=c?b.domId+"-"+c.guid:b.domId,g=document.createElement("paper-dialog");g.setAttribute("modal","modal");g.setAttribute("id",d);e.node.appendChild(g);Polymer.dom.flush();Polymer.updateStyles();render.buildPage(b,
c);console.log("Showing dialog: "+d);e=util.polyElm(d);e.node.refit();e.node.constrain();e.node.center();e.node.open()}else"dialogsTabName"==b.tabId?(render.buildPage(b),a.setTopDialogStackItem(b),e=document.querySelector("#dialogIronPages"),e.select(b.domId),b.visible=!0,util.setVisibility("#"+b.domId,b.visible)):render.buildPage(b)},setTopDialogStackItem:function(b){var c=a.dialogStack.indexOfObject(b);0!=c&&(-1!=c&&a.dialogStack.splice(c,1),0==a.dialogStack.length?a.dialogStack.push(b):a.dialogStack.splice(0,
0,b),console.log("DialogStack: "+a.dialogPath()))},dialogPath:function(){for(var b="",c=0;c<a.dialogStack.length;c++)b+="/"+a.dialogStack[c].domId;return b},openDialog:function(b){return a.changePage(b)},cancelDialog:function(b){1==a.dialogStack.length&&(meta64.selectTab("mainTabName"),view.scrollToSelectedNode());var c=a.dialogMap[b];null==c?console.log("unable to find dialog page for: "+b):(b=a.dialogStack.indexOfObject(c),-1!=b&&a.dialogStack.splice(b,1),c.visible=!1,util.setVisibility("#"+c.domId,
c.visible),0<a.dialogStack.length&&a.changePage(a.dialogStack[0]))},popup:function(){render.buildPage(popupMenuPg);$("#"+popupMenuPg.domId).popup("open")},isNodeBlackListed:function(b){if(!a.inSimpleMode())return!1;for(var c in a.simpleModeNodePrefixBlackList)if(a.simpleModeNodePrefixBlackList.hasOwnProperty(c)&&b.name.startsWith(c))return!0;return!1},getSelectedNodeUidsArray:function(){var b=[],c=0,e;for(e in a.selectedNodes)a.selectedNodes.hasOwnProperty(e)&&(b[c++]=e);return b},getSelectedNodeIdsArray:function(){var b=
[],c=0,e;a.selectedNodes?console.log("selectedNode count: "+a.selectedNodes.length):console.log("no selected nodes.");for(e in a.selectedNodes)if(a.selectedNodes.hasOwnProperty(e)){var d=a.uidToNodeMap[e];d?b[c++]=d.id:console.log("unable to find uidToNodeMap for uid="+e)}return b},getSelectedNodesArray:function(){var b=[],c=0,e;for(e in a.selectedNodes)a.selectedNodes.hasOwnProperty(e)&&(b[c++]=a.uidToNodeMap[e]);return b},clearSelectedNodes:function(){a.selectedNodes={}},updateNodeInfoResponse:function(a,
c){var e="",d=!1;a.owners&&$.each(a.owners,function(a,b){0<e.length&&(e+=",");b===meta64.userName&&(d=!0);e+=b});if(0<e.length){c.owner=e;var g=$("#ownerDisplay"+c.uid);g.html(" (Manager: "+e+")");d?util.changeOrAddClass(g,"created-by-other","created-by-me"):util.changeOrAddClass(g,"created-by-me","created-by-other")}},updateNodeInfo:function(b){var c=util.json("getNodePrivileges",{nodeId:b.id,includeAcl:!1,includeOwners:!0});c.completes.then(function(){a.updateNodeInfoResponse(c.response,b)})},getNodeFromId:function(b){return a.idToNodeMap[b]},
getPathOfUid:function(b){var c=a.uidToNodeMap[b];return c?c.path:"[path error. invalid uid: "+b+"]"},getHighlightedNode:function(){return a.parentUidToFocusNodeMap[a.currentNodeUid]},highlightRowById:function(b,c){var e=a.getNodeFromId(b);e?a.highlightNode(e,c):console.log("highlightRowById failed to find id: "+b)},highlightNode:function(b,c){if(b){var e=!1,d=a.parentUidToFocusNodeMap[a.currentNodeUid];d&&(d.uid===b.uid?e=!0:(d=d.uid+"_row",d=$("#"+d),util.changeOrAddClass(d,"active-row","inactive-row")));
e||(a.parentUidToFocusNodeMap[a.currentNodeUid]=b,d=b.uid+"_row",d=$("#"+d),util.changeOrAddClass(d,"inactive-row","active-row"));c&&view.scrollToSelectedNode()}},refreshAllGuiEnablement:function(){var b=util.getPropertyCount(a.selectedNodes),c=a.getHighlightedNode();util.setEnablement("navLogoutButton",!a.isAnonUser);util.setEnablement("navLoginButton",a.isAnonUser);util.setEnablement("openSignupPgButton",a.isAnonUser);util.setEnablement("navHomeButton",!0);util.setEnablement("propsToggleButton",
a.currentNode&&!a.isAnonUser);util.setEnablement("deletePropertyButton",!a.isAnonUser);var e=a.currentNode&&!a.isAnonUser;util.setEnablement("editModeButton",e);util.setEnablement("moveSelNodesButton",!a.isAnonUser&&0<b);util.setEnablement("deleteSelNodesButton",!a.isAnonUser&&0<b);util.setEnablement("clearSelectionsButton",!a.isAnonUser&&0<b);util.setEnablement("moveSelNodesButton",!a.isAnonUser&&0<b);util.setEnablement("finishMovingSelNodesButton",!a.isAnonUser&&null!=edit.nodesToMove);util.setEnablement("changePasswordPgButton",
!a.isAnonUser);util.setEnablement("accountPreferencesPgButton",!a.isAnonUser);util.setEnablement("insertBookWarAndPeaceButton",a.isAdminUser);util.setEnablement("manageAttachmentsButton",!a.isAnonUser&&null!=c);util.setEnablement("editNodeSharingButton",!a.isAnonUser&&null!=c);util.setEnablement("renameNodePgButton",!a.isAnonUser&&null!=c);util.setEnablement("searchPgButton",!a.isAnonUser&&null!=c);util.setEnablement("timelineButton",!a.isAnonUser&&null!=c);util.setEnablement("showServerInfoButton",
a.isAdminUser);util.setEnablement("showFullNodeUrlButton",null!=c);util.setEnablement("refreshPageButton",!a.isAnonUser);util.setEnablement("findSharedNodesButton",!a.isAnonUser&&null!=c);util.setVisibility("editModeButton",e);util.setVisibility("insertBookWarAndPeaceButton",a.isAdminUser);util.setVisibility("propsToggleButton",!a.isAnonUser);util.setVisibility("openLoginPgButton",a.isAnonUser);util.setVisibility("navLogoutButton",!a.isAnonUser);util.setVisibility("openSignupPgButton",a.isAnonUser);
util.setVisibility("mainMenuSearchButton",!a.isAnonUser&&null!=c);util.setVisibility("mainMenuTimelineButton",!a.isAnonUser&&null!=c);Polymer.dom.flush();Polymer.updateStyles()},getSingleSelectedNode:function(){for(var b in a.selectedNodes)if(a.selectedNodes.hasOwnProperty(b))return a.uidToNodeMap[b];return null},getOrdinalOfNode:function(b){if(!a.currentNodeData||!a.currentNodeData.children)return-1;for(var c=0;c<a.currentNodeData.children.length;c++)if(b.id===a.currentNodeData.children[c].id)return c;
...
c={daylightSavingsTime:(new Date).dst()?!0:!1,inherit:function(a,b){b.prototype=Object.create(a.prototype)},json:function(c,f,g){if(a)console.log("offline: ignoring call for "+c);else{var h=Polymer.dom(this.root).querySelector("#ironAjax");h.url=postTargetUrl+c;h.verbose=!0;h.body=JSON.stringify(f);h.method="POST";h.contentType="application/json";h.handleAs="json";h.debounceDuration="300";b++;var l=h.generateRequest();l.completes.then(function(){b--;"function"==typeof g&&g(l.response)},function(){b--;
console.log("Error in util.json");if("403"==l.status)console.log("Not logged in detected in util."),a=!0,d||(d=!0,messagePg.alert("Session timed out. Page will refresh.")),$(window).off("beforeunload"),window.location.href=window.location.origin;else{var c="Server request failed.\n\n";try{c+="Status: "+l.statusText+"\n",c+="Code: "+l.status+"\n"}catch(e){}messagePg.alert(c)}});return l}},ajaxReady:function(a){return 0<b?(console.log("Ignoring requests: "+a+". Ajax currently in progress."),!1):!0},
