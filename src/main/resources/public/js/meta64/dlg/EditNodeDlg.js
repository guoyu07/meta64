...
EditNodeDlg_.populateEditNodePg = function() {
	...
	if (edit.editNode) {
		...
		$.each(editOrderedProps, function(index, prop) {
			...
			/*
			 * this is the way (for now) that we make sure this node won't be attempted to be saved. If it has RdOnly_
			 * prefix it won't be found by the saving logic.
			 */
...
EditNodeDlg_.saveExistingNode = function() {
	...
	while (true) {
		...
		if (meta64.fieldIdToPropMap.hasOwnProperty(fieldId)) {
			...
			/*
			 * Since the Readonly ones are prefixed with RdOnly_ in front of fieldId, those won't exist and
			 * elementExists bypasses them
			 */
